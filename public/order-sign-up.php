...
if(checkIsStringSetPost('firstname') && checkIsStringSetPost('email'))
{
  ...
  while($row = mysqli_fetch_assoc($check))
  {
  	if($email==$row['email'])
  	{
                echo "<script>alert(' The entered email arleady has a store, please try again with different email or try to login with your previous store');window.location='order-login.php';</script>";exit;
  	}
  }
  $date = date("Y-m-d");
  ...
  {
      echo "<script>alert(' There is an error while creating the store, please try again ');window.location='order-login.php';</script>";
  }
}
else 
{

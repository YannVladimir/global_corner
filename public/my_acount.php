                                       ...
                                       <?php
                                          if(isset($_SESSION['id']))
                                          {
                                             ...
                                             while($row = mysqli_fetch_assoc($res))
                                             {
                                               ...
                                               echo "<tr class='odd gradeX'><td>Service</td><td><strong>{$row['title']}</strong></td><td><strong>{$row['sub_category']}</strong></td><td>{$interested} <br> <a href='order-sub-category.php?id={$row['subcategory_id']}&type=0&category={$row['sub_category']}'>View all</a></td><td>Votes:{$row['total_votes']}  <br> Rating: {$row['avg']} </td><td><form action='my_service.php' method='GET'><input type='text' name='id' value='{$row['id']}' class='hidden'/><input type='submit' value='View Details'/></form></td><td><form action='delete.php' method='POST'><input type='text' class='hidden' name='_token' value='{$_SESSION['_token']}'><input type='text' name='id' value='{$row['id']}' class='hidden'/><input type='submit' value='Remove'/></form></td></tr> ";
                                             }
